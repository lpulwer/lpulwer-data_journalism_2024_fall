...
```{r}
# Party registrations
New_Reg_2024 <- read.csv("data/New Registration 2024.csv") |>
...
```{r}
# Removals
Rem_2024 <- read.csv("data/Removal 2024.csv")  |>
  rename(Description = X) |>
  drop_na()
```
...
```{r}
# Changes
Change_2024 <- read.csv("data/Change 2024.csv") |>
 rename(County = X)
```

```{r}
# Current registrations
Act_Reg_2024 <- read.csv("data/Active Registrations 2024.csv") 
  county_value <- c("ALLEGANY", "ANNE ARUNDEL", "BALTIMORE CITY", "BALTIMORE CO.", "CALVERT", "CAROLINE", "CARROLL", "CECIL", "CHARLES", "DORCHESTER", "FREDERICK", 	
"GARRETT", "HARFORD", "HOWARD", "KENT", "MONTGOMERY", "PR. GEORGE'S", "QUEEN ANNE'S", "ST. MARY'S", "SOMERSET", "TALBOT", "WASHINGTON", "WICOMICO", "WORCESTER", "TOTAL") 
Act_Reg_2024_new <- Act_Reg_2024 |>
mutate(County = county_value)
...
```{r}
# Party registrations
New_Reg_2022 <- read_csv("data/New_Registration_2022.csv") |>
...
```{r}
# Removals
Rem_2022 <- read.csv("data/Removal 2022.csv") |>
rename(Description = X) |>
  drop_na()
```
...
```{r}
# Changes
Change_2022 <- read.csv("data/Change 2022.csv") |>
 rename(County = X)
```

```{r}
# Current registrations
Act_Reg_2022 <- read_csv("data/Active_Registration_2022_NEW.csv")
county_value <- c("ALLEGANY", "ANNE ARUNDEL", "BALTIMORE CITY", "BALTIMORE CO.", "CALVERT", "CAROLINE", "CARROLL", "CECIL", "CHARLES", "DORCHESTER", "FREDERICK", 	
"GARRETT", "HARFORD", "HOWARD", "KENT", "MONTGOMERY", "PR. GEORGE'S", "QUEEN ANNE'S", "ST. MARY'S", "SOMERSET", "TALBOT", "WASHINGTON", "WICOMICO", "WORCESTER", "TOTAL") 
Act_Reg_2022_new <- Act_Reg_2022 |>
mutate(County = county_value)
...
Less registered Democrats and Republicans voters, more registered Unaffiliated voters.



```{r}
Act_Reg_2024_Nocommas <- Act_Reg_2024_new |>
 mutate(
  DEM = gsub(",", "", DEM),
  REP = gsub(",", "", REP),
  UNAF = gsub(",", "", UNAF),
...
```{r}
Act_Reg_2024_numeric <- Act_Reg_2024_Nocommas |>
  mutate(
...
total_2024 <- nrow(Act_Reg_2024_numeric)
  
#I don't know if I was sleeping in class or not paying attention, but should it really, be this complicated? Been googlin' for hours.  
```

```{r}
Percentage_Voter_2024 <- Act_Reg_2024_numeric  |>
  slice(total_2024) |>
  reframe(
    percent_DEM = (DEM / TOTAL),
    percent_REP = (REP / TOTAL),
    percent_UNAF = (UNAF / TOTAL)
  )
```

```{r}
Act_Reg_NoCommas <- Act_Reg_2022_new |>
  mutate(
  DEM = gsub(",", "", DEM),
  REP = gsub(",", "", REP),
  UNAF = gsub(",", "", UNAF),
...
```{r}
Act_Reg_2022_numeric <- Act_Reg_NoCommas |>
  mutate(
    DEM = as.numeric(as.character(DEM)),
    REP = as.numeric(as.character(REP)),
    UNAF = as.numeric(as.character(UNAF)),
...
total_2022 <- nrow(Act_Reg_2022_numeric)
```

```{r}
Percentage_Voter_2022 <- Act_Reg_2022_numeric  |>
  slice(total_2024) |>
  reframe(
    percent_DEM = (DEM / TOTAL),
    percent_REP = (REP / TOTAL),
...
```{r}
Percent_change_DEM <- (0.5245 - 0.5408) / 0.5408

Percent_change_REP <- (0.2385 - 0.2400) / 0.2400

Percent_change_UNAF <- (0.2183 - 0.2022) / 0.2022
...
A2. Prince George's County with about 17%.

```{r}
Act_Reg_2024_numeric |>
  arrange(TOTAL)
  
Act_Reg_2022_numeric |>
arrange(TOTAL)
  

Combine_2020_2024 <- Act_Reg_2022_numeric |>
  select(County, UNAF) |>
  left_join(Act_Reg_2024_numeric |> select(County,UNAF), by = "County")

percent_change_UNAF <- Combine_2020_2024 |>
  mutate(percent_increase_UNAF = (UNAF.y - UNAF.x) / UNAF.x)
```
