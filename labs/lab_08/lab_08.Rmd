...
The county with highest percentage of total absentee ballots returned of total ballots sent is Worcester county. 
...
md_top_counties <- md_absentee_ballots |>
  mutate(Percentage_County = total_received/total_sent) |>
 arrange(desc(Percentage_County)) |>
  select(county_name, Percentage_County) |> 
  head(10)
...
md_top_counties |>
ggplot() +
  geom_bar(aes(x=reorder(county_name, Percentage_County), weight=Percentage_County)) +
  coord_flip() + 
labs(
...
The top 10 counties with the lowest percentage of ballots returned by Democratic voters are Baltimore City, Montgomery, Prince George's, Frederick, Baltimore County, Charles Howard, Cecil, Anne Arundel and Carroll. 

```{r}
md_absentee_ballots_dem_rep <- md_absentee_ballots  |> 
mutate(Percentage_Rep = rep_received/rep_sent) |>
mutate(Percentage_Dem = dem_received/dem_sent)
 
md_absentee_ballots_dem <- md_absentee_ballots_dem_rep |> 
  arrange(Percentage_Dem) |> 
  select(county_name, Percentage_Dem, Percentage_Rep) |>
  head(10)

Difference_Dem_Rep <- md_absentee_ballots_dem |>
  mutate(Diff_Party = Percentage_Dem - Percentage_Rep)

md_absentee_ballots_dem_only <- md_absentee_ballots_dem |>
 select(county_name, Percentage_Dem)

md_absentee_ballots_dem_only |> 
ggplot() +
  geom_bar(aes(x=county_name, weight=Percentage_Dem)) +
  coord_flip() + 
labs(
    title="Counties with Highest Percentage of Absentee Ballots Returned",
    x = "County",
...
**A3** 
It shows that Democrats have a higher voting engagement (by far) than Republicans and other voters. 
The Democrats' line also shows that the closer to election day, the more absentee ballots were counted. 

This does match with absentee voting habits as Republicans are known to vote less via mail than Democrats (especially ever since the 2020 election)
...
Frederick_County |>
ggplot() + 
  geom_line(aes(x=date, y=dem_received, color="dem_received"), size = 1) +
  geom_line(aes(x=date, y=rep_received, color="rep_received"), size = 1) +
  geom_line(aes(x=date, y=oth_received, color="oth_received"), size = 1)  +
  labs(title = "Democrats Lead in Absentee Voting, While Republicans Lag Behind",
       x = "Date",
       y = "Number of Received Ballots",
       color = "Party") +
  scale_color_manual(values = c("dem_received" = "blue", "rep_received" = "red", "oth_received" = "green")) +
  theme_minimal()
...
I can't see everything as most of the counties are crowded (too lazy to fix that) However, I do see that Anne Arundel, Prince George's and Baltimore County and especially Montgomery are the counties with most sent ballots. The line shows that there is a positive correlation between ballots sent and ballots received meaning as the number of ballots sent increase so does the number of ballots received increase.

Now I could talk about correlation all day. I just wanna mention one thing that I've learned during my Undergrad: Correlation does not imply causation. 
```{r}
MD_Ballots <- md_absentee_ballots |>
  select(`COUNTY NAME`, `TOTAL SENT`, `TOTAL RECEIVED`) 

MD_Ballots |>
ggplot(aes(x=`TOTAL SENT`, y=`TOTAL RECEIVED`, label=`COUNTY NAME`)) +
  geom_point(color = "red") +
   geom_text(vjust = -0.5, hjust = 1, size = 2) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Scatterplot of Ballots Sent vs. Ballots Received",
       x = "Ballots Sent",
       y = "Ballots Received", 
caption = "source: Maryland State Board of Elections")
