...
```{r}
Harris_combined <-  bind_rows(list(July_Harris, August_Harris))
```
...
Join_Voter_Harris <- New_Voters |> inner_join(Harris_combined, join_by(FirstName == first_name, LastName == last_name, Address == address_one, ResidentialZip == zip, ResidentialCity == city))
...
I chose FirstName, LastName, Address, ResidentialZip, ResidentialCity to narrow down the results. While FirstName and LastName help narrow down the results to a view individuals, there is still a slight chance of different people sharing the same name. That's why I also added Address, ResidentialZip and ResidentialCity to make sure we can differentiate. One problem might be that eventough every column matches, it could still be the case that this is not the same person (e.g. different person from the same household making a contribution). In addition, there could also be variations in how names or addresses were recorded (e.g. St. vs. Street)
...
A2. Based on the join, I've found that 236 voters who registered in 2024 have made donations to Harris. Among them, Brittany Musolino, Eric Snipe, and Sophia Hoffer-Perkins are the top donors, each contributing a total of 7 donations since July 20.
...
A3. The gender breakdown shows that 172 females and 140 males made donations to Harris. In terms of party affiliation, 292 donors identified as Democrats, while 4 identified as Republicans. 2 donations came from the Libertarian Party and 2 from other parties. One contribution came from the NLM party (not sure what party that is)Last but not least, 23 contributions came from unaffiliated parties (I guess that's what the UNA stands for?)
...
A4: Uhhhhhm. What is the average amount of donations that were made by new voters? (Is that interesting enough?)

If yes, then the average donation would be 	
59.23$ per new voter.
```{r}
Av_donations <- Join_Voter_Harris |>
summarise(Av_donations = mean(amount))
