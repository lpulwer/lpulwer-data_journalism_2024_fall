...
**Answer** With mutate(), it will create a new column to the existing table. So to the data named "primary_24", I added a new column named "percent_election_day". With this, I calculated what percentage of votes were cast on election day.
...
**Answer** It arranges the column "percent_election_day" from the biggest percentage to the lowest one. The two first rows result in 100% which means that all the votes in these two counties were given on election day.
...
**Answer** Without assuming any correlation we can see that the first rows mainly consist of democrats, suggesting that democrats take the largest part when it comes to voting on election day.
...
**str_to_lower -\>** puts letters into lower case

**str_to_title** -\> capitalizes every first letter of a word
...
**Answer** Create a column called amount_catgeory telling R that when a specific range of amount is given it should show me to corresponding
...
So that we can see all the the other columns from the prince_georges variable that are not included selected_prince_georges
...
```{r}
or_prince_georges <- primary_24 |> c
...
**Answer:** The date type was changed into YYYY-MM-DD format
...
**Answer** Each entry in the month column shows the start date of a month. The data shows us the total expenditure per month.
