...
**In the first part of the code, we're making changes to the old dataframe hence not creating a separate dataframe. With the mutate function, we create a new variable called UNAF_DIFF which calculates the change of unaffiliated voters in Maryland (UNAF_2024 - UNAF_2022 is the formula). In the next step, we create a new dataframe called maryland_top_counties using md_voters. In this new dataframe, we want to arrange variable UNAF_DIFF in a descending order. In addition, we only want to see the two variables County and UNAF_DIFF. Finally, the "head(10)" says that it should only give us the first 10 rows hence the top 10 counties.**
...
**Nothing :'(. Unless a blank canvas means something to you.**
...
**A bar chart with each county named on the x-axis and the numbers of unaffiliated voters (the change) on the y-axis.**
...
**The bars are ordered from the county with the lowest change to the county with the highest change of unaffiliated voters.**
...
**The counties are now aligned vertically while the numbers are aligned horizontally.**
...
**The gray background has turned white. Wow.**
...
**The theme of the chart has changed. The background is blue. Double wow.**
...
**We added a title to the chart. We alos changed the label for the x and y axis. We also added a caption below the chart. Triple wow.**
...
**We grouped the dataframe by date and then counted how many contributions were made on that day.**
...
**Below we can see a line chart. The x-axis shows the dates while the y-axis shows the numbers of contributions made on that date. The problem with this illustration is that R only displays the months but not the exact dates. Not good for reading!**
...
**Now the x-axis does not show each date separately, but rather R grouped the dates into weeks. So the chart now shows the number of contributions per week.**
...
**The labels on the x-axis are slightly hty tilted so we can see them better. I think with hjust we make sure that there is also enough space between each of the labels.**
