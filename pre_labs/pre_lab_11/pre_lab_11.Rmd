...
The column names are not very handy as they are super long and R doesn't recognize the columns as numbers (There are commas in there) 
```{r}
...
It's symbolizes the columns. 2 means second column, 3 means third column and so on and so forth.
...
html_text will find any text character in the tags. It shows the name of the committees as well as the url link. 
```{r}
...
It creates a dataframe out of the html with text and the url link. Two columns were created. A name and url column with all committee names and the url to that committee. 
