...
**Answer** obs=131395; variables=24
...
For "last_name" it's <chr> and for "amount" it's <dbl>. Last_name represents the last names of the contributors (duh) and amount represents the amount each contributor has given.
...
**Answer** 
Gives the numbers of contributors who contributed on a specific date.
...
```{r}
contribs_by_name <- maryland_winred_contributions |>
  group_by(date) |>
  summarise(
    count_contribs = n()
  ) |>
...
It arranges from the highest number of contributors who contributed on a specific date to to the lowest number of contributors who contributed on a specific date. 
...
Mutliple columns are grouped together. 
...
It shows the total amount of contributions. 
...
**Answer**
It gives the average of the amount of donations. 
Earmarked for HOGAN FOR MARYLAND INC. (C00869016) -> 83.27590719	
### Task 11
...
It gives the minimum amount and maximum amount for each city. Total amount is sorted from highest to lowest number. 
