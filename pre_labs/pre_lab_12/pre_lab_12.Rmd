...
Gives date, title, url and the text of the press releases. 
...
It tells R that I only want these set of words to appear once. 
...
Creates a list of unique words of the selected text.  
...
The most used word is "the". So interesting *sarcasm*
...
Yes. Words with substance are now included. Health is the most often mentioned word (excluding the stop_words)
...
Year 2022 included 841 mentions of the world health while 2024 included only 438 mentions of health. The most mentioned word is baltimore in 2024. 
...
It gives us a list of the two most mentioned words that come together. So we get more nuanced ideas like human rights and mental health rather than just words. 
...
Health care, amount awarded and purpose funds were the most mentioned joined words. The percentages of these words, however, are very small. 
...
Stem_words gives a number of variations of a word like drive (diver, drove etc.), while lemmatize_words gives one variation of word (because they probably mean the same). For press release the one variation would probably be more interesting as drive, drove basically mean the same thing (same concept) and it wouldn't make sense to differentiate them. If we want to be more specific, we would want to use stem_words.
...
The dataframe with lemmatization is more useful than the dataframe without the lemmatization because it includes words that basically refer to the same concept (funding and funds). Why should we differentiate between them when they basically mean the same thing? 
...
Decided that 547 "support" words were positive. I'm not sure if abnormal is necessarily a negative word. Depends on the context (as always...)
