...
There are geometrical columns included like Shape_Length, Shape_Area and Shape.   
```{r}
...
Each precinct_name has a unique precinct ID. Again it includes the columns Shape_Length, Shape_Area and Shape. It also includes the Polling-ID. It basically shows all sorts of places where polling station are, although the coordinates won't help an insignificant mortal like me knowing where the exact location is (for that I need text or a map)
...
There we go! Each precinct is shown as certain shape and it also includes the boundaries. The counties are shaped according to coordinates and show how the counties relate to one another.
```{r}
pg_precincts |>
  ggplot() +
  geom_sf() +
...
It provides an overview of the columns, including a geometry column and other intriguing columns with values that appear quite scary.
```{r}
...
**Answer**
Oh, I guess STATEFP represents the code for a state and 24 stands for Maryland. So now it shows us a dataframe with info for Maryland only.  
```{r}
...
We can see the outline of Maryland. The counties are positioned accurately according to their coordinates and are also accurately shaped to one another. 
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
So the FIPS code represents the a specific county. 
We narrowed it down even further from counties to a specific county (Prince George's). We can now see the boundaries of the precincts. 
...
Shows dataframe containing columns precinct, total_polls, total_early, total_absentee, total_provisional and total_eligible. Each row represents a precinct. 
...
The new dataframe contains all columns from the pg_precincts dataset, including the geometry column for mapping, along with the turnout-related columns from pg_turnout. 
...
Each precinct is represented as a distinct shape. The precincts are filled with colors that reflect the eligible voters in each precinct, with higher numbers being shaded lighter, while lower numbers are shaded darker. However, there are also some precincts thata re not colored at all. 
```{r}
ggplot() +
...
Contains columns such as county, name of parties, and totals.
...
It shows the estimated population for each Maryland County. It also includes a GEOID which is a unique identifier for each county. 
...
So we basically get an output that contains both the estimated population data as well as geomatric data marked as MULTIPOLYGON.
...
The output shows us all 24 Maryland counties and numbers of voters for a party per county. It also gives us the total number of voters per county. 
...
There's no "Maryland" or "County" included in the county column. The dataframe shows us the follwoing three columns we need: County, Population and Geometry. 
```{r}
...
We now have a combined dataframe with county, population, DEM, REP, LIB, OTH, UNA, NLM, GRN, TOTAL and geometry. However, some values are NA. 
...
Any "St. Mary's" has been named to "Saint Mary's" and any "Baltimore" has been renamed to "Baltimore County" 
...
One last column we added to our dataframe is the "voters_per_10k" column. We calculated that by dividing the TOTAL by the population times 10k. Top counties include Worcester, Queen Anne's and Talbot county.  
...
This map seems familiar. Is it a choropleth map by any chance?
It shows Maryland's Counties shaded to its voters per 10,000 population (The lighter the color, the higher the number. Shouldn't it be the other way around though?) 
...
Colorblind people will cry out of joy now. Yeah!
...
Ok, either I'm blind or the code didn't work. I don't really see a change.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We created the "una_voters_per_10k" column which calculates the unaffiliated votes by the population times 10k.
...
The Choropleth map shows Maryland's Counties shaded to its unaffiliated voters per 10,000 population (Again, the lighter the color the higher the number)

We can see that Howard, Anne Arundel and Frederick County have the highest number of unaffiliated voters per 10k population. Meanwhile, Somerset, Prince George's and Garrett county have the lowest number of unaffiliated voters.
